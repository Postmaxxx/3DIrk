            ...
            return (
                <Fragment key={fiber._id}>
                    <div className={`cell col-name ${fiber._id === fibersState.selected ? "selected" : ""}`} onClick={e => onCellClick(fiber._id, '')} >
                        ...
                        <NavLink to={`../${fiber._id}`} className='button_blue button_sort' aria-label={lang === 'en' ? `Learn more about fiber ${fiber.name.en}` : `Узнать больше про материал ${fiber.name.ru}`}>
                    ...
                    {fibersProperties.map((property) => {                                                
                        return (
                            <Fragment key={property._id}>
                                {!listHiddenProps.includes(property._id) && //filter, priceGr won't shown in this table
                        ...
                        {fibersState.load.status === 'success' && 
                            <div className="table">
                                ...
                                <div className="cell row-name fixed-left selectors">
                                    {(filtered && !selectedMore) 
                                        && <button 
                                            className='button_blue button_reset' 
                                            aria-label={lang === 'en' ? 'Show all fibers' : 'Показать все материалы'} 
                                            onClick={clearSelected}>
                                                {lang === 'en' ? 'Show all' : 'Показать все'}
                                            </button>}
                                    {(!filtered || selectedMore) 
                                        && <button 
                                            className='button_blue button_compare' 
                                            aria-label={lang === 'en' ? 'Compare selected fibers' : 'Сравнить выбранные материалы'} 
                                            onClick={compareSelected}>
                                                {lang === 'en' ? 'Compare' : 'Сравнить'}
                                            </button>}
